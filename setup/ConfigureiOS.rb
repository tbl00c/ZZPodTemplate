module Pod
  ...
  class ConfigureIOS
    ...
    def perform
      ...
      # keep_demo = configurator.ask_with_answers("是否包含Demo工程", ["Yes", "No"]).to_sym
      keep_demo = "Yes"

      # framework = configurator.ask_with_answers("请选择猜测框架", ["Specta", "Kiwi", "None"]).to_sym
      # case framework
      #   when :specta
      ...
      # snapshots = configurator.ask_with_answers("是否添加视图测试框架", ["Yes", "No"]).to_sym
      # case snapshots
      #   when :yes
      ...
      loop do
        prefix = configurator.ask("请输入类名前缀")
