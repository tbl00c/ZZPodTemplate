...
module Pod
  class TemplateConfigurator
    ...
    def run
      ...
      # platform = self.ask_with_answers("请选择平台?", ["iOS", "macOS"]).to_sym
      platform = "iOS"
      ...
      #   when :ios
      #     framework = self.ask_with_answers("请选择语言?", ["Swift", "ObjC"]).to_sym
      #     case framework
      #       when :swift
    ...
    def rename_files_name
      ...
      file_names.each do |file_name|
        origin_name = file_name.clone
        new_name = file_name.clone
        new_name.gsub!("POD_", @pod_name)

        File.rename(origin_name, new_name)
      end
    end
